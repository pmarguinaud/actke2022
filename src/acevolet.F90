SUBROUTINE ACEVOLET (YDCST, YDML_PHY_MF, KIDIA, KFDIA, KLON, KTDIAT, KTDIAN, KLEV, PAPHI, PAPHIF, PAPRS, PAPRSF, PDELP, PECT,  &
& PKUROV, PR, PT, PU, PV, PUSLE, PPRODTH, PKCLS, PECTCLS, PECT1, PPRDY, PDIFF, PDISS)
  !-----------------------------------------------------------------------
  ! - INPUT  2D .
  ! - INPUT  1D .
  ! - OUTPUT 2D .
  ...
  REAL(KIND=JPRB) :: ZA(KLON, KLEV), ZE(KLON, KLEV), ZPA(KLON, KLEV), ZGKU(KLON, 0:KLEV), ZDET(KLON, KLEV), ZGKEF(KLON, KLEV),  &
  & ZCOR(KLON)
  
  
  INTEGER(KIND=JPIM) :: JLEV, JLON
  
  REAL(KIND=JPRB) :: ZB, ZCIS, ZD, ZD1, ZD2, ZD3, ZDPHI, ZDPHISRO, ZGSDP, ZMDP, ZRHOF, ZRTI, ZUSGDT, ZX, ZY, ZZDIS, ZZGKEF,  &
  & ZDPHI2
  ...
  & ADISI=>YDML_PHY_MF%YRPHY0%ADISI, ECTMAX=>YDML_PHY_MF%YRPHY0%ECTMAX, TSPHY=>YDML_PHY_MF%YRPHY2%TSPHY, RG=>YDCST%RG,  &
  ...
  DO JLON=KIDIA,KFDIA
    ...
    PDISS(JLON, KLEV - 1) = -ZZDIS*ZGSDP*(ADISI*ZA(JLON, KLEV) + ADISE*PECT(JLON, KLEV - 1))
  ...
  DO JLEV=KLEV - 2,KTDIAN,-1
    DO JLON=KIDIA,KFDIA
      ...
      PDISS(JLON, JLEV) = PDISS(JLON, JLEV)*(ADISI*ZA(JLON, JLEV + 1) + ADISE*PECT(JLON, JLEV))
    END DO
    ! JLON
  END DO
  ! JLEV
  ...
  DO JLON=KIDIA,KFDIA
    ...
    PDIFF(JLON, KLEV - 1) =  &
    & (ZPA(JLON, KLEV - 1)*(ZA(JLON, KLEV) - ZA(JLON, KLEV - 1)) - ZANKP1(JLON)*(PECTCLS(JLON) - ZA(JLON, KLEV)))*ZGSDP
  END DO
  ! JLON
  ...
  DO JLEV=KTDIAN,KLEV - 1
    DO JLON=KIDIA,KFDIA
      ZDET(JLON, JLEV) = MIN((ECTMAX - PECT(JLON, JLEV)) / TSPHY, MAX((ECTMIN - PECT(JLON, JLEV)) / TSPHY, PDIFF(JLON, JLEV) +  &
      & PDISS(JLON, JLEV) + PPRDY(JLON, JLEV) + PPRODTH(JLON, JLEV)))
  ...
  IF (LECTREP) THEN
    DO JLON=KIDIA,KFDIA
      ZDET(JLON, KLEV) = (MIN(ECTMAX, MAX(ECTMIN, PECT1(JLON, KLEV - 1))) - PECT(JLON, KLEV)) / TSPHY
    END DO
    ! JLON
  ELSE
    DO JLON=KIDIA,KFDIA
      ZDET(JLON, KLEV) = (MIN(ECTMAX, MAX(ECTMIN, PECTCLS(JLON))) - PECT(JLON, KLEV)) / TSPHY
    END DO
    ! JLON
  END IF
  ...
  DO JLON=KIDIA,KFDIA
    ...
    ZCOR(JLON) = ZDET(JLON, KLEV) / (SIGN(1._JPRB, ZDET(JLON, KLEV - 1))*MAX(ZEPS, ABS(ZDET(JLON, KLEV - 1))))
