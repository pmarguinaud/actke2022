SUBROUTINE ACBL89 (YDCST, YDPHY, YDPHY0, KIDIA, KFDIA, KLON, KTDIAN, KLEV, PAPHI, PAPHIF, PAPRS, PAPRSF, PT, PECT, PQV, PQICE,  &
& PQLI, PNLAB, PNLABCVP, PGZ0, PTS, PUSLE, PLMECT, PPHI3)
  !-----------------------------------------------------------------------
  ! - INPUT  2D .
  ! - INPUT  1D .
  ! - OUTPUT 2D .
  ...
  REAL(KIND=JPRB) :: ZDLDN, ZDLDN1, ZDLDN2, ZQV, ZDLUP, ZDLUP1, ZDLUP2, ZDPHI, ZEPSX, ZINCR, ZG2L2SLD2, ZGLDIS, ZGLKARMN,  &
  & ZGLMCBR, ZGLMINF, ZGLMIX, ZPHI3MAX, ZPREF, ZQC, ZTEST, ZTEST0, ZTESTM, ZUSX, ZX, ZZDTHVL, ZZDTHVLP, ZZTHVL, ZZTHVLP, ZQCS,  &
  & ZGZLCVPUP, ZGZLCVPDN, Z2SQRT2, ZLUP(KLON), ZLDN, ZLWK0, ZLWK1(KLON)
  ...
  & ALMAVE=>YDPHY0%ALMAVE, ARSC1=>YDPHY0%ARSC1, RATM=>YDCST%RATM, RETV=>YDCST%RETV, RG=>YDCST%RG, RKAPPA=>YDCST%RKAPPA,  &
    ...
    DO JJLEV=JLEV,KTDIAN + 1,-1
      IF (ZTESTM > 0.0_JPRB) THEN
        DO JLON=KIDIA,KFDIA
          ...
          ZDLUP2 = (-(ZUSX - 0.5_JPRB) + SIGN(1.0_JPRB, ZX)*SQRT(ABS((ZUSX - 0.5_JPRB)*(ZUSX - 0.5_JPRB) +  &
    ...
    DO JJLEV=JLEV + 1,KLEV - 1
      IF (ZTESTM > 0.0_JPRB) THEN
        ...
        DO JLON=KIDIA,KFDIA
          ...
          ZUSX = 1.0_JPRB / (SIGN(1.0_JPRB, ZX)*MAX(ZEPSX, ABS(ZX)))
          ZDLDN2 = (-(ZUSX - 0.5_JPRB) + SIGN(1.0_JPRB, ZX)*SQRT(ABS((ZUSX - 0.5_JPRB)*(ZUSX - 0.5_JPRB) +  &
  ...
  DO JLEV=KTDIAN,KLEV - 1
    ...
    DO JLON=KIDIA,KFDIA
      ...
      ZG2L2SLD2 = MAX(ZPHI3MAX, ARSC1*MIN(2.0_JPRB, ZGLMCBR*ZGLMCBR*ZDTHETA(JLON, JLEV + 1) / ZDPHI / PECT(JLON, JLEV) /  &
      & ZTHETAH(JLON, JLEV)))
