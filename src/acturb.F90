SUBROUTINE ACTURB (YDCST, YDPHY, YDPHY0, KIDIA, KFDIA, KLON, KTDIAT, KTDIAN, KLEV, PAPHI, PAPHIF, PAPRS, PAPRSF, PR, PT, PU, PV,  &
& PECT, PQV, LDCONV, PLSCPE, PLMECT, PPHI3, PCD, PCH, PGZ0, PTS, PQS, PQICE, PQLI, PKTROV, PKQROV, PKQLROV, PKUROV, PNBVNO,  &
& PPRODTH, PNEBS, PQCS, PL3F2, PGKCLS, PECTCLS)
  !-----------------------------------------------------------------------
  ! - INPUT  2D .
  ! - INPUT  1D .
  ! - INPUT/OUTPUT  2D .
  ...
  INTEGER(KIND=JPIM) :: IHCLPMAX, IHCLPMIN, IJLEVM1, IJLEVP1, INIV, INQ1, JLEV, JLON
  
  LOGICAL :: LLIMQ1
  
  REAL(KIND=JPRB) :: Z2B, Z3B, Z3BCF, ZA, ZAA, ZCE1, ZCIS, ZCK, ZCTO, ZDD, ZDELTQF, ZDELTQF1, ZDELTQF2, ZDELTQH, ZDI, ZDIFFC,  &
  & ZDIFFH, ZDLEWF, ZDLEWF1, ZDLEWF2, ZDPHI, ZDPHI0, ZDQLST, ZDQW, ZDS, ZDSTA, ZDT, ZDTETA, ZDTL, ZDU2, ZECTH, ZEPDELT, ZEPNEBS,  &
  & ZECTBLH, ZEPS, ZEPS1, ZEPSQ, ZEPSQ1, ZEPSV, ZEW, ZEW1, ZEW2, ZFACT, ZGALP2, ZGLMT2, ZGLMU2, ZGLT, ZGLTZ, ZGLU, ZGLUZ, ZGZ,  &
  & ZH, ZH1, ZH2, ZIGMAS, ZIGMAS2, ZINC, ZIS, ZLCPM1, ZLCPP1, ZLMECT, ZLOI, ZLOS, ZLSCPEF, ZLSCPEF1, ZLSCPEF2, ZLSCPEH, ZMAXQ1,  &
  & ZMODU, ZNEBLOW, ZPHI3MIN, ZPHMAX, ZPHMIN, ZPREF, ZPRODC, ZPRODH, ZQ11, ZQ1MAX, ZQ1MIN, ZQC, ZQLF, ZQCF1, ZQCF2, ZQLH, ZQLM1,  &
  & ZQLP1, ZQSATF, ZQSATF1, ZQSATF2, ZQSLTLF, ZQSLTLF1, ZQSLTLF2, ZQWF, ZQWF1, ZQWF2, ZQWH, ZRESUL, ZRH, ZRHOH, ZRIC, ZRICHF,  &
  & ZRIH, ZRLTLU, ZROSDPHI, ZRQZERO, ZRTI, ZSIGMAS, ZSIGMAS2, ZSRC, ZSTA, ZSURSAT, ZTETA, ZTF, ZTF1, ZTF2, ZTH, ZTHETAOT, ZTLF,  &
  & ZTLF1, ZTLF2, ZU, ZUSTAR2, ZWDIFF, ZWQW, ZWTL, ZZETF, ZZF0, ZZF1, ZZKTH, ZZLMF, ZZN1D, ZZQC, ZZRT, ZZT, ZL3F2, ZILIMQ1,  &
  & ZEPSIG, ZHTOP, ZHBOT, ZSIGCR, ZPRETURB
...
#include "fcttrm.func.h"
  ...
  ASSOCIATE (UPRETMIN=>YDPHY0%UPRETMIN, VKARMN=>YDPHY0%VKARMN, UPRETMAX=>YDPHY0%UPRETMAX, ARSB2=>YDPHY0%ARSB2,  &
  & ECTMIN=>YDPHY0%ECTMIN, AKN=>YDPHY0%AKN, ALPHAT=>YDPHY0%ALPHAT, AGRE1=>YDPHY0%AGRE1, AGRE2=>YDPHY0%AGRE2,  &
  & AJBUMIN=>YDPHY0%AJBUMIN, ALMAV=>YDPHY0%ALMAV, AECLS4=>YDPHY0%AECLS4, TURB=>YDPHY0%TURB, STTBMIN=>YDPHY0%STTBMIN,  &
  & AECLS3=>YDPHY0%AECLS3, RCOFLM=>YDPHY0%RCOFLM, GALP=>YDPHY0%GALP, UDECT=>YDPHY0%UDECT, USHEARM=>YDPHY0%USHEARM,  &
  & UCWSTAR=>YDPHY0%UCWSTAR, ARSC1=>YDPHY0%ARSC1, AGREF=>YDPHY0%AGREF, EDB=>YDPHY0%EDB, EDC=>YDPHY0%EDC, RICRET=>YDPHY0%RICRET,  &
  & EDD=>YDPHY0%EDD, USURIC=>YDPHY0%USURIC, GCVTURB=>YDPHY0%GCVTURB, RATM=>YDCST%RATM, RCPD=>YDCST%RCPD, RD=>YDCST%RD,  &
  & RDT=>YDCST%RDT, RETV=>YDCST%RETV, RG=>YDCST%RG, RKAPPA=>YDCST%RKAPPA, RPI=>YDCST%RPI, RTT=>YDCST%RTT, RV=>YDCST%RV,  &
  & LPBLE=>YDPHY%LPBLE, LNEIGE=>YDPHY%LNEIGE, LECTQ1=>YDPHY%LECTQ1, LECTFL0=>YDPHY%LECTFL0, LNEBECT=>YDPHY%LNEBECT,  &
  & LECTREP=>YDPHY%LECTREP, LDISTUR=>YDPHY%LDISTUR, LDIFCEXP=>YDPHY%LDIFCEXP, LCVTURB=>YDPHY%LCVTURB, LNEBRIC=>YDPHY%LNEBRIC)
  !-----------------------------------------------------------------------
  ...
  DO JLON=KIDIA,KFDIA
    ...
    ZRTI = 2.0_JPRB / (PR(JLON, KLEV)*PT(JLON, KLEV) + RKAPPA*ZDPHI0 + ZRS(JLON)*PTS(JLON))
    ZSTA = ZDPHI0*(PR(JLON, KLEV)*PT(JLON, KLEV) + RKAPPA*ZDPHI0 - ZRS(JLON)*PTS(JLON))*ZRTI
  ...
  DO JLEV=KTDIAT,KTDIAN - 1
    ...
    DO JLON=KIDIA,KFDIA
      ...
      ZDTETA = PR(JLON, JLEV)*PT(JLON, JLEV) - PR(JLON, JLEV + 1)*PT(JLON, JLEV + 1) + RKAPPA*ZDPHI0
  ...
  IF (LPBLE .or. .not.LECTREP) THEN
    ...
    DO JLON=KIDIA,KFDIA
      IF (ILEVBI(JLON) > 1 .and. ILEVBI(JLON) < KLEV) THEN
        ...
        ZBLH(JLON) = ZHBOT + (ZHTOP - ZHBOT) / (PECT(JLON, ILEVBI(JLON) - 1) - PECT(JLON, ILEVBI(JLON)))*(ZECTBLH - PECT(JLON,  &
        & ILEVBI(JLON)))
      ELSE IF (ILEVBI(JLON) == KLEV) THEN
        ZBLH(JLON) = (PAPHIF(JLON, KLEV) - PAPHI(JLON, KLEV)) / RG
      ELSE IF (ILEVBI(JLON) == 0) THEN
  ...
  IF (.not.LECTREP) THEN
    ...
    DO JLON=KIDIA,KFDIA
      ...
      PECTCLS(JLON) = MAX(ECTMIN, AECLS3*ZUSTAR(JLON)**2 + AECLS4*ZWSTAR(JLON)**2*(1.0_JPRB - ZSTAB(JLON)))
  ...
  DO JLEV=KTDIAN,KLEV - 1
    ...
    DO JLON=KIDIA,KFDIA
      ...
      IF (LCVTURB .and. ZDELTQH > 0._JPRB .and. PAPRSF(JLON, JLEV) < ZPRETURB .and. LDCONV(JLON, JLEV)) THEN
      ...
      IF (LECTQ1) THEN
        ...
        ZQ1MAX = ZDELTQH*ZDPHI / ZLMECT / SQRT(ARSB2*AKN*ALPHAT*ZPHI3MIN) / MAX(ABS(ZDIFFC), ZEPSQ1)
  ...
  IF (LPBLE) THEN
    ...
    DO JLEV=KLEV - 1,KTDIAN,-1
      DO JLON=KIDIA,KFDIA
        ...
        ZECTINT(JLON) = ZECTINT(JLON) + ICLA*PECT(JLON, JLEV)*(PAPHIF(JLON, JLEV) - PAPHIF(JLON, JLEV + 1))
        ZQCINT(JLON) =  &
        & ZQCINT(JLON) + ICLA*(PQLI(JLON, JLEV + 1) + PQICE(JLON, JLEV + 1))*(PAPHI(JLON, JLEV) - PAPHI(JLON, JLEV + 1))
      END DO
    END DO
    ...
    DO JLON=KIDIA,KFDIA
      IF (ILEVBI(JLON) > 0) THEN
        ...
        ZNUM = ZECTINT(JLON) + PECTCLS(JLON)*(PAPHIF(JLON, KLEV) - PAPHI(JLON, KLEV))
        ...
        IF (AGREF < 0._JPRB) THEN
          ...
          ZA = -AGREF*AGRE1*(1._JPRB - (1._JPRB / AGREF + 1._JPRB)*SIN(RPI / 2._JPRB*MAX(0._JPRB, MIN(1._JPRB, ((ZBICX - ZBI) /  &
          & (ZBICX - ZBIC)))))**2._JPRB)
          ZQCBLK = PQLI(JLON, ILEVT + 1) + PQICE(JLON, ILEVT + 1)
          ZA = ZA*(1._JPRB + AGRE2*PLSCPE(JLON, ILEVT)*ZQCBLK / MAX(ZTHETAVL(JLON, ILEVT) - ZTHETAVL(JLON, ILEVT + 1),  &
          & AJBUMIN*ZTHETA(JLON, ILEVT)))
        ELSE
          ZA = AGRE1*(1._JPRB + AGRE2*AGREF*PLSCPE(JLON, ILEVT)*ZQCBLK / MAX(ZTHETAVL(JLON, ILEVT) - ZTHETAVL(JLON, ILEVT + 1),  &
          & AJBUMIN*ZTHETA(JLON, ILEVT)))
        END IF
        ! (AGREF < 0.)
        ...
        ZBI = MAX(ZBI, ZBIC)
        ZGKENT = MAX(ZA*ZECTBLK*SQRT(ZECTBLK)*RG / ZLINV / ZBI, ZGKTH(JLON, ILEVT))
  ...
  IF (LDIFCEXP) THEN
    ...
    DO JLEV=KTDIAN,KLEV - 1
      DO JLON=KIDIA,KFDIA
        ...
        ZDQW = PQV(JLON, JLEV) - PQV(JLON, JLEV + 1) + PQLI(JLON, JLEV) - PQLI(JLON, JLEV + 1) + PQICE(JLON, JLEV) - PQICE(JLON,  &
        & JLEV + 1)
        ZDTL = PT(JLON, JLEV) - PT(JLON, JLEV + 1) - PLSCPE(JLON, JLEV)*(PQLI(JLON, JLEV) + PQICE(JLON, JLEV)) + PLSCPE(JLON,  &
        & JLEV + 1)*(PQLI(JLON, JLEV + 1) + PQICE(JLON, JLEV + 1))
  ...
  IF (LNEBECT) THEN
    ...
    DO JLEV=KLEV,KTDIAN,-1
      ...
      DO JLON=KIDIA,KFDIA
        ...
        ZDU2 = (PU(JLON, IJLEVM1) - PU(JLON, IJLEVP1)*REAL(INIV, kind=JPRB))**2 + (PV(JLON, IJLEVM1) - PV(JLON, IJLEVP1) &
        & *REAL(INIV, kind=JPRB))**2
        ...
        IF (LCVTURB .and. ZDELTQF > 0._JPRB .and. PAPRSF(JLON, JLEV) < ZPRETURB .and. LDCONV(JLON, JLEV)) THEN
        ...
        IF (LECTQ1) THEN
          ...
          ZGALP2 = GALP*GALP / TURB
          ZPHI3MIN = 1.0_JPRB / (1.0_JPRB + ARSC1*ZGALP2)
          ZQ1MAX = ZDELTQF*ZDPHI / ZZLMF / SQRT(ARSB2*AKN*ALPHAT*ZPHI3MIN) / MAX(ABS(ZDIFFC), ZEPSQ1)
          ZQ1MAX = SIGN(MAX(ABS(ZQ1MAX), ZEPSQ1), ZQ1MAX)
          ZQ1MAX = SIGN(MIN(ABS(ZQ1MAX), ZMAXQ1), ZQ1MAX)
          ZQ1MAX = ZILIMQ1*ZQ1MAX - (1.0_JPRB - ZILIMQ1)*ZMAXQ1
